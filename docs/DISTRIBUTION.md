...
Each published binary must have a matching immutable Git tag such as `v1.0.6`. The tag contains the source, CMake files, pinned vcpkg metadata, tests, resources, packaging scripts, and notices used for that release. GitHub's automatically generated tag archives may be used as source downloads.
